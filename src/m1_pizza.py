...
def run_test_generate_points_on_circle():
    ...
    print('Actual:  ', answer)
    #Test 3
    expected = [rg.Point(90, 50),  # All numbers are approximate.
                rg.Point(110, 50)]
...
def run_test_draw_points_on_circle():
    ...
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(120, 130), 70)
    circle.fill_color = 'black'
    draw_points_on_circle(window, circle, 70, 'purple')
    window.continue_on_mouse_click()
...
def run_test_pizza():
    ...
    title = 'my test'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(150, 200), 77)
    circle.outline_thickness = 2
    pizza(window, circle, 70, 'yellow',3)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    circle = rg.Circle(rg.Point(200, 250), 70)
    circle.outline_thickness = 2
    polygon(window, circle, 7, 'blue', 3)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(number_of_segments):
        a = generate_points_on_circle(circle,number_of_segments + 1)
        line = rg.Line(a[k], a[k + 1])
        line.thickness = thickness
        line.color = color
        line.attach_to(window)
        aa = rg.Line(a[number_of_segments], a[0])
...
def run_test_fancy_polygon():
    ...
    title = ('FANCY POLYGON test 3:  20 lime green lines on blue circle,'
             + ' hops = 7.')
    window = rg.RoseWindow(700, 800, title)

    circle = rg.Circle(rg.Point(330, 400), 100)
    circle.fill_color = 'light blue'
    fancy_polygon(window, circle, 51, 25, 'white', 1)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(number_of_lines):
        ...
        if k + hops_to_next_point < number_of_lines:
            line = rg.Line(a[k], a[k + hops_to_next_point])
            line.arrow = 'last'
            line.color = color
            line.thickness = thickness
            line.attach_to(window)
        else:
            line = rg.Line(a[k], a[(k - number_of_lines) + hops_to_next_point])
            line.arrow = 'last'
            line.color = color
            line.thickness = thickness
            line.attach_to(window)
